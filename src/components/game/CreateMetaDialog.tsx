...
export function CreateMetaDialog() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTitle(''); setVolume(''); setReward(''); setDays('30');
    setOpen(false);
  };
  ...
  const categories: { value: Category; label: string; colorClass: string }[] = [
    ...
    { value: 'trabalho', label: '🟠 Trabalho', colorClass: 'border-game-orange bg-game-orange/10 text-game-orange' },
  ...
  if (!open) {
    return (
      <button
        ...
        className="w-full bg-gradient-card rounded-lg p-4 shadow-game-card border border-dashed border-primary/40 hover:border-primary hover:shadow-glow-gold transition-all duration-300 flex items-center justify-center gap-2 text-primary font-display text-sm tracking-wider"
      >
  ...
  return (
    <div className="bg-gradient-card rounded-lg p-5 shadow-game-card border border-primary/30 animate-slide-up">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <label className="text-xs font-display tracking-wider text-muted-foreground block mb-1.5">NOME DA META</label>
          <input
            value={title}
        ...
        <div>
          <label className="text-xs font-display tracking-wider text-muted-foreground block mb-1.5">CATEGORIA</label>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="text-xs font-display tracking-wider text-muted-foreground block mb-1.5">PRAZO (DIAS)</label>
            <input
              type="number"
              value={days}
          ...
          <div>
            <label className="text-xs font-display tracking-wider text-muted-foreground block mb-1.5">VOLUME (OPCIONAL)</label>
            <input
              value={volume}
        ...
        <div>
          <label className="text-xs font-display tracking-wider text-muted-foreground block mb-1.5">RECOMPENSA (OPCIONAL)</label>
          <input
            value={reward}
